...
%% Constant Properties
    ...
    methods (Static)
        ...
        %
        % OK = mic.Utils.checkNum(ARGUMENTS)
        ...
        %   the property
        %
        % out = mic.Utils.isprop(oThis,sProperty)
        %
        ...
        % out = mic.Utils.isMethod(oThis,sMethod)
        %
        ...
        %   OUT = mic.Utils.ut(hUIChild, hUIParent)
        %
        ...
        % PANELHEIGHT 
        %   out = mic.Utils.panelHeight(u8Edits)
        %       u8Edits: number of stacked edits in panel
        %
        % See also UIH, UIW, UT, LT2LB, UICONTROLY
            out = mic.Utils.dPanelTopPad + ...
                mic.Utils.dPanelBotPad + ...
                u8Edits*(mic.Utils.dEDITHEIGHT + mic.Utils.dLABELHEIGHT) + ...
                (u8Edits-1)*mic.Utils.dEditPad;
        end

        function absolute_position = get_screenPos(h)
        % GET_SCREENPOS Gets the absolute position of the handle
        %   position = mic.Utils.get_screenPos(handle), where position is a
        ...
        % See also GET_EDITABLE, GET_NEXTEDIT, SCROLL_INCREMENT
            ...
            if ~ishandle(h)
                error('mic.Utils.get_screenPos argument is not an handle !')
            end

            if  ~isequal(size(h),[1 1])
                error('mic.Utils.get_screenPos does not accept vectors of handle as argument')
            end
        ...
        % See also GET_SCREENPOS, GET_NEXTEDIT, SCROLL_INCREMENT
            ...
            for i=1:length(eboxes)
                ...
                xyi = mic.Utils.get_screenPos(eboxes(i));
        ...
        % See also GET_SCREENPOS, GET_EDITABLE, GET_NEXTEDIT, SCROLL_INCREMENT
            if (strcmp(evt.Modifier,'control') == 1)
                switch evt.Key
                    case 'i'
                        direction = 'up';
                        mic.Utils.get_nextEdit(src, direction);
                    case 'k'
                        direction = 'down';
                        mic.Utils.get_nextEdit(src, direction);
                    case 'j'
        ...
        %   set(gcf,'WindowScrollWheelFcn',@mic.Utils.scroll_increment)
        ...
        % See also GET_SCREENPOS, GET_EDITABLE, GET_NEXTEDIT, KEYBOARD_NAVIGATION
            ...
            edit_handle = mic.Utils.get_editable(src, get(src,'CurrentPoint'));
            ...
            if ~isempty(edit_handle) && ishandle(edit_handle)
                ...
                try %tricky, because there is no typecheck on the box so far %FIXME to change
                    ...
                    disp('mic.Utils.scroll_increment : an error occured !')
                end
            end
        end
                    ...
                    if exist(fullfile(cPath, [ceName{1}, '.mat']), 'file') ~= 0
                        ...
                        switch cAns
                            ...
                            otherwise
                                ...
                                % Return empty
                                % cReturn = '';
                                cReturn = mic.Utils.saveAs(cName, cPath);
                        end
                    ...
                    if isempty(strmatch(ceName{1}, ceOptions, 'exact'))
                        ...
                        switch cAns
                            ...
                            otherwise
                                ...
                                % Return empty
                                % cReturn = '';
                                cReturn = mic.Utils.listSaveAs(cName, ceOptions);
                        end 
                    end
                else
